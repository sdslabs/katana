...
func GitServer(c *fiber.Ctx) error {
	...
	writer.WriteField("domain", LoadBalancer+":18080")
	writer.WriteField("ssh_port", "22")
	writer.WriteField("http_port", "3000")
	writer.WriteField("app_url", "http://"+ LoadBalancer+":18080"+":3000/")
	...
	// Create the request
	req, err := http.NewRequest("POST", "http://"+ LoadBalancer+":18080"+"/install", &requestBody)
	if err != nil {
		return err
	}
