...
func envVariables(gogs string, pwd string, podNamespace string) {
	watch, _ := kubeClientset.CoreV1().Pods(podNamespace).Watch(context.Background(), metav1.ListOptions{})
	for event := range watch.ResultChan() {
		...
		if p.Status.Phase != "Pending" {
			podName := "katana-team-master-pod-0"
			log.Println("Pod created")
			command := []string{"bash", "-c", "echo 'export GOGS=" + gogs + "' >> /etc/profile"}
			utils.PodExecutor(command, kubeClientset, kubeConfig, podName, podNamespace)
